...
public class RefuseOrderItemDelegate implements ItemViewDelegate<OrderBean> {
    private MultiItemTypeAdapter.OnItemClickListener mOnItemClickListener;



    public RefuseOrderItemDelegate(MultiItemTypeAdapter.OnItemClickListener mOnItemClickListener) {
        this.mOnItemClickListener = mOnItemClickListener;
    }

    @Override
    public int getItemViewLayoutId() {
        return R.layout.list_refuse_order;
    }

    @Override
    public boolean isForViewType(OrderBean item, int position) {

        return item.orderstate == OrderStateEnum.REFUSE_ORDER.getCode();
    }
    ...
    public void convert(final ViewHolder holder, OrderBean orderBean, final int position) {
        ...
        holder.setOnClickListener(R.id.btn_refuse_delete, new View.OnClickListener() {
            @Override
